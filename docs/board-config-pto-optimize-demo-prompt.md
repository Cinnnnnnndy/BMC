...
- **G2 告警流卡片化 + 流全貌 + 高密度**：传感器由「一行一条 + 176px 竖排事件列」（显示效率低）改为
  **紧凑节点卡横向 wrap 流排**——每卡 = `图标+名+门限/状态 tag+数据源点+chevron` 头 + 事件内联 chip 条，
  一屏可并排多张（实测底部 dock 一行 4 张），事件以小 chip 呈现整条链路告警；
  点选的卡**占满整行 + 主色内描边**（呼应参考图 3「选中节点蓝框」）并就地展开门限/事件配置。
...
**R6（.sr 全量导入 + 器件列表 .sr 派生，2026-07-22）已落地**：
- `alarmKnowledge` 加通用 `sr_state`（离散状态）量，承载无专用量对应的 .sr 离散传感器。
- `srSeed` 扩展为导入门限 + 离散全量：门限→单板温度器件，离散→「系统状态 System」器件（带 .sr 事件，如 PowerButton→ButtonPowerButtonPressed）。实测 ExpBoard_1 由 2 温度传感器扩到 **7 传感器 / 2 监控对象（TempBoard + System）/ 10 告警**。
- `TopologyView` 器件列表 = mock 可添加器件 ∪ **.sr 派生器件**（选板即播种）；实测「系统状态 System」自动进入本板器件列表，可下钻器件级告警。
- 仍有边界：.sr 离散事件与传感器为间接关联（经信号/Accessor），本次按对象名归属尽力而为；CSR 生成与原 .sr 的完全往返保真尚未校验。
