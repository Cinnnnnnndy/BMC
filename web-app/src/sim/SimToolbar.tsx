...
const STATUS_COLOR = { normal: '#16a34a', warning: '#d97706', error: '#dc2626' };
const MUTED = '#9aa3b2';   // light-theme secondary text

// ─── Button primitives ────────────────────────────────────────────────────
const BASE: React.CSSProperties = {
  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4,
  background: '#f1f3f7', border: '1px solid #dfe3ea',
  color: '#4a5260', borderRadius: 5, padding: '3px 10px',
...
const DIVIDER = (
  <div style={{ width: 1, height: 20, background: 'rgba(60,80,120,0.14)', flexShrink: 0, margin: '0 2px' }} />
);
    ...
    <div style={{
      ...
      {selBus && selBusColor && (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 10, flexShrink: 0, padding: '2px 10px', background: `${selBusColor.css}12`, border: `1px solid ${selBusColor.css}30`, borderRadius: 6 }}>
            ...
            <span><span style={{ color: MUTED }}>带宽 </span><span style={{ color: selBusColor.css }}>{selBus.bandwidth}</span></span>
            <span style={{ color: MUTED }}>|</span>
            <span><span style={{ color: MUTED }}>频率 </span><span style={{ color: selBusColor.css }}>{selBus.frequency}</span></span>
      ...
      {histLen > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, maxWidth: 160 }}>
          <span style={{ fontSize: 9, color: MUTED, whiteSpace: 'nowrap' }}>历史</span>
          <input
            type="range"
            min={0}
            max={histLen - 1}
          ...
          />
          <span style={{ fontSize: 9, color: MUTED, fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0 }}>
        <span style={{ fontSize: 9, color: MUTED, userSelect: 'none' }}>
          3D · 滚轮缩放 · 拖动旋转 · 右键平移
        </span>
      </div>
    </div>
