...
const BOARD_CONNECTIONS = [
  ...
  { fromType: 'IEU', toType: 'PSR',     label: 'I²C 管理总线',         color: '#475569', style: 'dashed' as const },
  { fromType: 'EXU', toType: 'PSR',     label: 'I²C 管理总线',         color: '#475569', style: 'dashed' as const },
  { fromType: 'SEU', toType: 'PSR',     label: 'I²C 管理总线',         color: '#475569', style: 'dashed' as const },
  { fromType: 'BCU', toType: 'IEU',     label: 'PCIe Hisport↔UBCDD',  color: '#3b82f6', style: 'solid' as const },
  { fromType: 'BCU', toType: 'EXU',     label: 'PCIe Hisport↔OCP',    color: '#60a5fa', style: 'solid' as const },
  { fromType: 'IEU', toType: 'SEU',     label: 'PCIe CEM↔UBC',        color: '#f97316', style: 'solid' as const },
  { fromType: 'IEU', toType: 'NICCard', label: 'PCIe CEM',             color: '#ec4899', style: 'solid' as const },
      ...
      {groups.slice(0, 5).map(({ busId, busType, chips }) => (
        <div key={busId} style={{ marginBottom: 4 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 3 }}>
            <div style={{ height: 2, width: 8, background: busType === 'hisport' ? '#0891b2' : '#0284c7' }} />
            <span style={{ fontSize: 9, color: busType === 'hisport' ? '#38bdf8' : '#475569', fontFamily: 'monospace' }}>
            ...
            {chips.map(c => {
              ...
              return (
                <div key={c.id} title={c.id + (c.address != null ? ` 0x${c.address.toString(16).toUpperCase()}` : '')}
                  ...
                  <div style={{
                    width: 22, height: 16, borderRadius: c.type === 'connector' ? 2 : 3,
                    background: color + '33', border: `1px solid ${color}88`,
                    boxShadow: showSoft && fruChips.has(c.id) ? `0 0 5px ${color}` :
                  ...
                  <div style={{ fontSize: 7, color: '#374151', marginTop: 8, maxWidth: 28, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {c.id.replace(/^(?:I2cMux_Pca\w+_|I2cMux_|Connector_PCIE_|Connector_Com|Connector_|Chip_|Smc_|Eeprom_)/, '').slice(0, 10)}
                  </div>
                </div>
              );
            })}
...
function ConnectionLines({
  ...
  const connPaths = useMemo(() => {
    ...
    for (const conn of BOARD_CONNECTIONS) {
      ...
      result.push({ key, d, color: conn.color, style: conn.style, colorName, fromType: conn.fromType, toType: conn.toType });
    }
    return result;
      ...
      {connPaths.map(cp => {
        ...
        return (
          ...
          >
            ...
            {/* Visible line */}
            <path
              d={cp.d} fill="none" stroke={cp.color}
...
function ExpandedConnectionLines({ boards, positions, showI2C, showPCIe, highlightUids }: {
  ...
  const paths = useMemo(() => {
    const result: Array<{ key: string; d: string; color: string; style: 'solid' | 'dashed'; opacity: number }> = [];
    ...
    for (const conn of BOARD_CONNECTIONS) {
      ...
      for (const fb of fromBoards) {
        for (const tb of toBoards) {
          ...
          const tx = tp.x;
          const ty = tp.y + BOARD_H / 2;
          const cp1x = fx + Math.abs(tx - fx) * 0.45;
          const cp2x = tx - Math.abs(tx - fx) * 0.45;
          const d = `M ${fx} ${fy} C ${cp1x} ${fy} ${cp2x} ${ty} ${tx} ${ty}`;
          const isActive = highlightUids.length === 0 || (highlightUids.includes(fb.uid) && highlightUids.includes(tb.uid));
          result.push({ key: `${fb.uid}|${tb.uid}`, d, color: conn.color, style: conn.style, opacity: isActive ? 0.7 : 0.08 });
        }
      }
    }
    return result;
  ...
  return (
    <svg style={{ position: 'absolute', top: 0, left: 0, overflow: 'visible', zIndex: 5, pointerEvents: 'none' }} width={maxX} height={maxY}>
      {paths.map(p => (
        <path key={p.key} d={p.d} fill="none" stroke={p.color}
          strokeWidth={1.5} strokeDasharray={p.style === 'dashed' ? '4,3' : undefined}
          opacity={p.opacity} />
      ))}
    </svg>
  );
