---
version: alpha
name: openUBMC Studio
description: Dark-first developer workstation design system for openUBMC Studio modules, graph surfaces, operator workbenches, and review previews. Model architecture gallery pages use the openPangu-2.0-Flash shared model-graph profile: modelArchitectureColormap(), true neutral tensor fills, transparent parent containers, consistent node typography, left-side Parameter/Weight inputs, right-side State/Cache auxiliaries, readable fit, and .openubmc-studio-model-architecture-stage.
colors:
  primary: "#0077FF"
...
# openUBMC Studio DESIGN.md

## 1. Visual Theme & Atmosphere

openUBMC Studio is not a marketing site. It is a developer workstation for understanding what data becomes across source, pass, runtime, and hardware views.
...
openUBMC Studio has three major surface types. They must not be styled as if they are the same thing.
...
openUBMC Studio has reusable graph, timeline, and hardware visualization primitives that are more specific than base components. These are pattern sources, not decorative examples.
...
- every pattern must expose a stable consumer entry: CSS class names for static visual primitives, and a `window.OpenUBMCStudio*Pattern` or `window.OpenUBMCStudio*` helper for DOM/SVG/canvas generation, drag behavior, zoom, routing, persistence, or synchronized state
...
- `ide-frame`: shared openUBMC Studio IDE framework shell registered and previewed with standalone and VS Code webview host modes; content-free slots only; standalone host mode includes the shared gradient/aura + translucent blurred pane skin by default
...
Misalignment is one of the fastest ways openUBMC Studio looks inconsistent.

## 8. Depth & Elevation

Dark openUBMC Studio should rely on:
...
When asking an AI agent to build openUBMC Studio UI, use directions like:

- “Use openUBMC Studio’s dark-first developer workstation style”
