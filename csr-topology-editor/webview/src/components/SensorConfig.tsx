...
export function SensorConfig({ csr, onChange }: Props) {
  ...
  const chipOptions = Object.keys(obj).filter((k) =>
    k.startsWith('Chip_') || k.startsWith('Pca') || k.startsWith('Ads') || k.startsWith('Eeprom')
  );
    ...
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      >
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <button
            onClick={() => { setTab('scanner'); setSelected(null); }}
            style={{
              flex: 1,
              padding: 8,
              background: tab === 'scanner' ? '#0e639c' : '#333',
              color: '#fff',
              border: 'none',
              borderRadius: 4,
              cursor: 'pointer',
          ...
          <button
            onClick={() => { setTab('threshold'); setSelected(null); }}
            style={{
              flex: 1,
              padding: 8,
              background: tab === 'threshold' ? '#0e639c' : '#333',
              color: '#fff',
              border: 'none',
              borderRadius: 4,
              cursor: 'pointer',
        ...
        {list.map((e) => (
          <div
            ...
            style={{
              padding: 8,
              marginBottom: 4,
              background: selected === e.id ? '#0e639c33' : '#252526',
              borderRadius: 4,
              cursor: 'pointer',
              fontSize: 12,
            }}
      ...
      <main style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
        {selected && list.find((e) => e.id === selected) ? (
          tab === 'scanner' ? (
            <ScannerForm
              id={selected}
              def={list.find((e) => e.id === selected)!.def}
              chipOptions={chipOptions}
              onUpdate={(f, v) => handleUpdate(selected, f, v)}
...
function ScannerForm({
  ...
  return (
    <div>
      ...
      {[
        { key: 'Chip', label: 'Chip', options: chipRefs },
        { key: 'Offset', label: 'Offset', type: 'number' },
        { key: 'Size', label: 'Size', type: 'number' },
        { key: 'Mask', label: 'Mask', type: 'number' },
        { key: 'Type', label: 'Type', type: 'number' },
        { key: 'Period', label: 'Period (ms)', type: 'number' },
      ...
      <button
        onClick={onDelete}
        style={{ marginTop: 16, padding: '8px 16px', background: '#c53030', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}
      >
        删除
      </button>
    </div>
...
function ThresholdSensorForm({
  ...
  const refOptions = objectKeys.map((k) => ({ value: `#/${k}.Value`, label: k }));
  const readingOptions = objectKeys
    .filter((k) => k.startsWith('Scanner_'))
    .map((k) => ({ value: `<=/${k}.Value`, label: k }));
  ...
  return (
    <div>
      ...
      {[
        ...
        { key: 'ReadingStatus', label: 'ReadingStatus', options: refOptions },
        { key: 'EntityId', label: 'EntityId', options: objectKeys.filter((k) => k.startsWith('Entity_')).map((k) => ({ value: `<=/${k}.Id`, label: k })) },
        { key: 'EntityInstance', label: 'EntityInstance', options: objectKeys.filter((k) => k.startsWith('Entity_')).map((k) => ({ value: `<=/${k}.Instance`, label: k })) },
